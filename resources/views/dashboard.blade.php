...
<header class="container">
    <div class="row justify-content-center">
        @if (session('success'))
            <div x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 2000)" class="alert alert-success alert-dismissible fade show my-2 w-50" role="alert">
                {{ session('success') }}
        ...
        @if (session('error'))
            <div x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 2000)" class="alert alert-danger alert-dismissible fade show my-2 w-50" role="alert">
                {{ session('error') }}
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>
        @endif
...
<script>
    ...
    document.getElementById('form-search-id').addEventListener('submit', function(event) {
        event.preventDefault();
        buscarUsuarioPorID();
    });
</script>
